...
- [ ] Every `connector: mcp` or `connector: api` tool has provenance: `source`/`upstream`, and `status: third-party` where applicable — **warning**.
...
- [ ] `connector: mcp` tools have a real `server/mcp.json` — **error** if missing. A TOOL.md is a declaration, not a server; without this the tool cannot be launched at all.
- [ ] Any tool with `authority.never` rules has an enforcement anchor, regardless of `connector` — **error** if missing. This is not mcp-specific: every connector type assumes real code exists somewhere that can actually stop the forbidden action, not just a TOOL.md saying it's forbidden. Accepted anchors:
  - `connector: mcp` — a `server/proxy/` that intercepts every call and applies each rule before forwarding to the real backend.
  - `connector: api` / `connector: local` — a non-empty `scripts/` folder holding the wrapper/enforcement code, **or** a `provenance.vendored` path that resolves to the real backing implementation (e.g. a pointer into a sibling API repo's endpoint/service files that actually enforce the rule).
