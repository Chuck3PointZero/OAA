...
function validateDomain(domain, graph, findings) {
...
function validateConcept(concept, domain, graph, findings, seenNames) {
    const src = domain.sourcePath;
    // Name
    if (!concept.name) {
        findings.push({
...
function validateToolMap(toolMap, graph, findings) {
    ...
    if (graph.domains.length > 0) {
        for (const [conceptName, { domain }] of graph.allConcepts) {
            const hasConcept = toolMap.maps.some((m) => m.conceptOrProperty === conceptName);
            if (!hasConcept) {
                findings.push({
                    severity: "gap",
