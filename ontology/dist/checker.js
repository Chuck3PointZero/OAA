...
export function checkPrograms(programs) {
    const findings = [];
    const symbols = {
        types: new Map(),
        enums: new Map(),
...
// ---------------------------------------------------------------------------
function collectDeclaration(stmt, src, symbols, findings) {
    switch (stmt.kind) {
        case "TypeDecl":
...
function checkDuplicate(name, symbols, src, loc, findings) {
    if (symbols.types.has(name) ||
...
function buildIdentityTypeIndex(symbols) {
    const index = new Map();
    ...
    switch (stmt.kind) {
        ...
        case "EntityDecl": {
            ...
            for (const prop of stmt.properties) {
                ...
                if (prop.constraint) {
                    validateExpr(prop.constraint, propNames, symbols, src, stmt.name, findings);
                }
            }
            ...
            for (const con of stmt.constraints) {
                validateExpr(con.expr, propNames, symbols, src, stmt.name, findings);
            }
            break;
        }
        case "DerivedEntityDecl": {
            ...
            const propNames = new Set(baseEntry.decl.properties.map((p) => p.name));
            validateExpr(stmt.where, propNames, symbols, src, stmt.name, findings);
            break;
        }
        case "MapDecl": {
            for (const entry of stmt.entries) {
                if (entry.target === "implicit")
                    continue;
                ...
                if (propPart && entityEntry.decl.kind === "EntityDecl") {
                    const hasProp = entityEntry.decl.properties.some((p) => p.name === propPart);
                    if (!hasProp) {
                        findings.push({
                            severity: "error",
                            check: "Map Reference",
            ...
            for (const entityName of symbols.entities.keys()) {
                ...
                if (decl.kind === "DerivedEntityDecl")
                    continue; // derived -> optional to map
                const mapped = stmt.entries.some((e) => e.source === entityName || e.source.startsWith(entityName + "."));
                if (!mapped) {
                    findings.push({
                        severity: "gap",
                        check: "Map Coverage",
...
// Expression checker
// ---------------------------------------------------------------------------
function validateExpr(expr, propNames, symbols, src, entityName, findings) {
    switch (expr.kind) {
        case "BinaryExpr":
        ...
        case "IdentRef":
            if (!propNames.has(expr.name)) {
                const isEnumValue = [...symbols.enums.values()].some((e) => e.values.includes(expr.name));
