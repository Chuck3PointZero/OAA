...
// Public API
// ---------------------------------------------------------------------------
export function emitSchemaSQL(programs, symbols) {
    const lines = [];
...
export function propToCol(name) {
...
export function exprToSQL(expr, enumValues = new Set()) {
    switch (expr.kind) {
        case "BinaryExpr": {
            const opMap = {
                "and": "AND", "or": "OR",
...
// Internal helpers
// ---------------------------------------------------------------------------
function mapPropertyType(typeName, symbols) {
    if (symbols.types.has(typeName)) {
        return mapBuiltIn(symbols.types.get(typeName).base);
    }
    if (symbols.enums.has(typeName))
        return "TEXT";
    if (symbols.entities.has(typeName))
        return "TEXT"; // FK to another entity stored as TEXT
    return mapBuiltIn(typeName);
}
function mapBuiltIn(t) {
    switch (t) {
...
function renderTable(entity, symbols) {
    const out = [];
    if (entity.doc)
        out.push("-- " + entity.doc);
...
function renderView(d, enumValues) {
    const out = [];
    if (d.doc)
        out.push("-- " + d.doc);
