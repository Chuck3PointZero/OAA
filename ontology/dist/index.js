...
const { version: ONTOLOGY_VERSION } = _req("../package.json");
import { loadPrograms } from "./loader.js";
import { checkPrograms } from "./checker.js";
...
let store = null;
let storeRootDir = null;
const toolDefinitions = [
    {
        name: "compile_ontology",
        description: "Parse all .rel files in the workspace, type-check them, then compile ONTOLOGY.md. " +
    ...
    },
    {
        ...
        description: "Parse and type-check all .rel files without writing any output. " +
    ...
    {
        name: "get_concepts",
        description: "Return the concept catalog from the workspace .rel files — entities, derived concepts, " +
    ...
    {
        name: "compile_schema",
        description: "Emit schema.sql from the .rel source files — CREATE TABLE for each base entity, " +
    ...
    {
        name: "write_entity",
        description: "Upsert an entity instance into the SQLite store. Validates the kind and field names " +
    ...
    {
        name: "query_concept",
        description: "Query entity instances from the store. Works for both base entities (SELECT * FROM Entity) " +
...
const server = new Server({ name: "oaa-ontology", version: ONTOLOGY_VERSION }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const a = (args ?? {});
    const rootDir = typeof a.rootDir === "string" ? resolve(a.rootDir) : ROOT_DIR;
    switch (name) {
        case "compile_ontology": {
            try {
                ...
                if (errors.length > 0) {
                    return text("Cannot compile — " + errors.length + " error(s) must be fixed first:\n\n" +
                        errors.map((e) => "  [" + e.check + "] " + e.source + ":" + e.line + "\n    " + e.message).join("\n") +
                        "\n\nRun validate_ontology for full details.");
                }
                const outputPath = typeof a.outputPath === "string"
        ...
        case "validate_ontology": {
            try {
                ...
                const out = ["Verdict: " + result.verdict, ""];
        ...
        case "get_concepts": {
            try {
                ...
                for (const [entityName, { decl, sourcePath }] of symbols.entities) {
                    if (filterName && entityName !== filterName)
                        continue;
