...
export function compileToMarkdown(programs, symbols, outputPath) {
    const now = new Date().toISOString();
    const lines = [];
    const allStmts = programs.flatMap((p) => p.statements);
    const types = allStmts.filter((s) => s.kind === "TypeDecl");
    const enums = allStmts.filter((s) => s.kind === "EnumDecl");
    const entities = allStmts.filter((s) => s.kind === "EntityDecl");
    const derived = allStmts.filter((s) => s.kind === "DerivedEntityDecl");
    const maps = allStmts.filter((s) => s.kind === "MapDecl");
    ...
    lines.push("");
    lines.push("This ontology defines the canonical vocabulary for this organization's business domain. " +
    ...
    if (derived.length > 0)
        lines.push("- [Derived Concepts](#derived-concepts)");
    if (enums.length > 0)
        lines.push("- [Value Types and Enumerations](#value-types-and-enumerations)");
    if (maps.length > 0)
        lines.push("- [Tool Vocabulary Maps](#tool-vocabulary-maps)");
    ...
    if (derived.length > 0) {
        ...
        lines.push("");
        lines.push("Derived concepts are subtypes defined by logical rules over base entities. " +
    ...
    if (maps.length > 0) {
        ...
        lines.push("");
        lines.push("Business concept names map to API vocabulary for each tool. " +
    ...
    for (const entity of entities) {
        ...
        lines.push("| [" + entity.name + "](#" + anchor + ") | entity | " + (entity.doc ?? "") + " |");
    }
    for (const d of derived) {
        const anchor = d.name.toLowerCase();
        lines.push("| [" + d.name + "](#" + anchor + ") | derived (extends " + d.base + ") | " + (d.doc ?? "") + " |");
    }
    lines.push("");
...
function renderEntity(entity) {
    ...
    if (entity.properties.length > 0) {
        ...
        for (const prop of entity.properties) {
            const constraintStr = prop.constraint
                ...
                : "";
            lines.push("| `" + prop.name + "` | " + prop.type + " | " + constraintStr + " | " + (prop.doc ?? "") + " |");
        }
        lines.push("");
    }
    if (entity.relations.length > 0) {
        ...
        for (const rel of entity.relations) {
            ...
            lines.push("| `" + rel.name + "` | " + targetLink + " | `" + rel.cardinality + "` | " + (rel.doc ?? "") + " |");
        }
        lines.push("");
    }
...
function renderMap(map, symbols) {
    const lines = [];
    lines.push("### " + map.tool);
    lines.push("");
    if (map.doc) {
    ...
    const mappedConcepts = new Set(map.entries.map((e) => e.source.split(".")[0]));
    const unmapped = [...symbols.entities.keys()].filter((name) => {
        const entry = symbols.entities.get(name);
        return entry?.decl.kind === "EntityDecl" && !mappedConcepts.has(name);
    });
    if (unmapped.length > 0) {
        lines.push("_Unmapped entities (no equivalent in this tool): " + unmapped.join(", ") + "_");
        lines.push("");
    }
    if (map.notes) {
