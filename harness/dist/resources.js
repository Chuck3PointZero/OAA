 ...
 */
export function listResources(rootDir) {
    const resources = [];
    // Always include the catalog
    resources.push({
        uri: "oaa://index.json",
        name: "OAA Node Catalog",
        description: "Catalog of all OAA nodes (agents, roles, skills, tools) in this workspace.",
 ...
 */
export function readResource(uri, rootDir) {
...
function readNodeByName(nodeName, rootDir) {
    const patterns = [
        "**/{AGENT.md,ROLE.md,SKILL.md,TOOL.md}",
        "**/*.{agent,role,skill,tool}.md",
    ];
...
function readCompiledChain(agentName, rootDir) {
    ...
    for (const f of agentFiles) {
        ...
        if (name === agentName || name.toLowerCase() === agentName.toLowerCase()) {
            ...
            if (!existsSync(agentsPath)) {
                throw new Error(`AGENTS.md not found for "${agentName}". Run compile_agent first.`);
            }
            return {
                mimeType: "text/markdown",
