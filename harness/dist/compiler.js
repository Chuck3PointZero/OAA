 ...
 * the tree entirely.
 */
export function findNodeFile(nodePath, baseDir) {
        ...
        if (statSync(abs).isDirectory()) {
            ...
            for (const [kind, filename] of Object.entries(TYPED_FILENAMES)) {
                ...
                if (existsSync(candidate)) {
                    return { filePath: candidate, kind: kind };
                }
            }
        }
        else {
 ...
 */
export function parseNode(filePath, kind) {
 ...
 */
export function resolveRequires(node, rootDir, visited = new Set(), depth = 0) {
    if (depth > 20)
        return []; // cycle guard
 ...
 */
export function resolveChain(agentName, rootDir) {
    // Search for the agent node
    const candidates = glob.sync(`**/{AGENT.md,*.agent.md}`, { cwd: rootDir, absolute: true, ignore: ["**/node_modules/**"] });
    let agentFile = null;
...
// ---------------------------------------------------------------------------
export function composeAuthority(chain) {
    const never = new Set();
    const decideSets = [];
    const escalates = new Set();
    const owns = new Set();
    const doesNotOwn = new Set();
    const envVars = new Set();
    // Tools contribute never constraints
    for (const tool of chain.tools) {
        const auth = tool.frontmatter.authority;
        if (auth?.never)
            auth.never.forEach((n) => never.add(n));
        if (tool.frontmatter.env)
            envVars.add(tool.frontmatter.env);
    }
    ...
    for (const role of chain.roles) {
        ...
        if (auth.never)
            auth.never.forEach((n) => never.add(n));
        if (auth.decides)
            decideSets.push(auth.decides);
        if (auth.escalates)
            auth.escalates.forEach((e) => escalates.add(e));
        if (auth.owns)
            auth.owns.forEach((o) => owns.add(o));
    }
    ...
    const decidesUnion = new Set();
    for (const set of decideSets) {
        for (const item of set)
            decidesUnion.add(item);
    }
...
// ---------------------------------------------------------------------------
export function renderAgentsMd(chain, authority, rootDir) {
    ...
    lines.push("");
    lines.push(`<!-- Generated by @oaa/harness on ${now}. Do not edit by hand. Source: ${relative(rootDir, agent.path)} -->`);
    lines.push("");
    // Identity
    lines.push("## Identity");
    lines.push("");
    ...
    lines.push("");
    lines.push("_Unlisted actions default to escalate. `never` overrides all other grants._");
    lines.push("");
    // Required env vars
 ...
 */
export function buildMcpConfig(chain, rootDir) {
    const mcpServers = {};
    const missing = [];
    for (const tool of chain.tools) {
        if (tool.frontmatter.connector !== "mcp")
            continue;
...
// agents.lock
// ---------------------------------------------------------------------------
function hashNode(filePath, dirPath) {
...
export function buildLockfile(chain, rootDir) {
    const nodes = {};
    ...
    for (const node of all) {
        ...
        const requires = (node.frontmatter.requires ||
...
export function compileAgent(agentName, rootDir) {
    const chain = resolveChain(agentName, rootDir);
    if (!chain) {
        throw new Error(`Agent "${agentName}" not found. Searched for AGENT.md or *.agent.md under ${rootDir}`);
    }
    ...
    const { config: mcpConfig, missing: missingMcpConfigs } = buildMcpConfig(chain, rootDir);
...
// Validation
// ---------------------------------------------------------------------------
export function validateGraph(rootDir) {
    const findings = [];
    ...
    const chains = [];
    const checkedTools = new Set(); // tool-level facts, dedupe across agents
    for (const af of agentFiles) {
        const agent = parseNode(af, "agent");
        // Check required frontmatter
        for (const field of ["kind", "name", "description"]) {
        ...
        for (const tool of chain.tools) {
            ...
            if (vendoredRel) {
                const vendoredAbs = resolve(toolDir, vendoredRel.replace(/^file:\/\/\.?\//, ""));
            ...
            if (never.length > 0) {
                ...
                if (!hasProxy && !hasScripts && !vendoredResolves) {
                    ...
                    if (gap?.reason) {
                        ...
                        const revisitRaw = gap.revisit;
                        const revisitDate = revisitRaw instanceof Date
                            ? revisitRaw
                            : revisitRaw
        ...
        const rawRequires = agent.frontmatter.requires ||
        ...
        if (agent.frontmatter.authority?.decides?.length) {
            findings.push({
                ...
                check: "Authority",
                file: af,
                message: "Agent declares `decides` — authority should be declared at role level",
...
// Status reader
// ---------------------------------------------------------------------------
export function getAgentStatus(agentName, rootDir) {
    const chain = resolveChain(agentName, rootDir);
    if (!chain) {
